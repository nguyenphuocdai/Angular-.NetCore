...
namespace DukptNet
{
    ...
    public static class Dukpt
    {
        ...
        {
            return Transform("TripleDES", true, bdk, (ksn & KsnMask) >> 16) << 64 | Transform(
                       "TripleDES",
                       true,
                       bdk ^ KeyMask,
                       (ksn & KsnMask) >> 16);
        ...
        private static BigInteger Transform(string name, bool encrypt, BigInteger key, BigInteger message)
        {
            ...
            {
                byte[] k = key.GetBytes();
                cipher.Key = new byte[Math.Max(0, GetNearestWholeMultiple(k.Length, 8) - k.Length)]
                    .Concat(key.GetBytes()).ToArray();
                ...
                using (ICryptoTransform crypto = encrypt ? cipher.CreateEncryptor() : cipher.CreateDecryptor())
                {
                    ...
                    data = new byte[Math.Max(0, GetNearestWholeMultiple(data.Length, 8) - data.Length)]
        ...
        public static byte[] Decrypt(string bdk, string ksn, byte[] encryptedData, DUKPTVariant variant)
        {
            ...
            return Transform("TripleDES", false, CreateSessionKey(bdk, ksn, variant), encryptedData.ToBigInteger())
                .GetBytes();
        }
